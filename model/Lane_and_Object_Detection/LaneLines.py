...
class LaneLines:
    ...
    def __init__(self):
        ...
        self.dir = []
        self.left_curve_img = mpimg.imread('model/Lane_and_Object_Detection/left_turn.png')
        self.right_curve_img = mpimg.imread('model/Lane_and_Object_Detection/right_turn.png')
        self.keep_straight_img = mpimg.imread('model/Lane_and_Object_Detection/straight.png')
    ...
    def fit_poly(self, img):
        ...
        threshold_percentage = 0.02  # for example, 2%
        total_pixels = img.shape[0] * img.shape[1]
        threshold = total_pixels * threshold_percentage

        leftx, lefty, rightx, righty, out_img = self.find_lane_pixels(img)

        # print(self.find_lane_pixels)

        if len(lefty) > threshold:
            self.left_fit = np.polyfit(lefty, leftx, 2)
        if len(righty) > threshold:
